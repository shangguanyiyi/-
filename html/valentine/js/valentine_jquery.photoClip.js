...
(function(root, factory) {
	...
	function photoClip(container, option) {
		...
		function clipImg() {
			...
			var bili=parseFloat(480/window.screen.width);
			// var bili=1;
			ctx.translate(curX - local.x / scale, curY - local.y / scale);
			console.log("local:"+local);
			console.log("local.x:"+local.x);
			console.log("local.y:"+local.y);
			var xCorrbinat=parseInt(local.x*bili);
			var yCorrbinat=parseInt(local.y*bili);
			$('#x').attr('value',xCorrbinat);
			$('#y').attr('value',yCorrbinat);
