...
var base_url="http://192.168.0.83/valentine/valentine/valentine.html";
//短信验证码倒计时
function setTime(){
		...
		success:function(data){
			...
			if(json.code=="success"){
				...
				if(json.isUplod==0){
					...
					// $('#third').hide();
					// $('#forth').show();
					window.location.href=base_url+"?userid=222";
				}
			}else{
				...
				$.ajax({
					...
					success:function(data){
						...
						if(json.code=="success"){
							...
							alert("登录成功后："+json.description);
							$('.yanzheng').hide();
							// $('#second').show();
							wetherUpLoadImg();
							// $('#second_clipArea').css("height",width);
							window.scrollTo(0,0);
						}else{
							alert(json.description);
						}
	 ...
	 //发送请求  剪切图片  放回原位
 	 $.ajax({
	        ...
	        success: function (data) {  
	            ...
	            userId=json.userid;
	            if(json.code=="success"){
	            	path=photo_url+json.image;
	            	console.log(path);
	            	$('#pic').attr("src",path);
	            	$('.mask').show();
	            	//share(userId);//分享的时候 带一个userid
	            	$('.fileInputContainer').hide();//上传成功之后按钮隐藏
	            }else{
	            	alert(json.description);
	            }
	        },   
...
/*
	...
	if(isWeixin){
		if(userCode=null||userCode==""){
			window.location.href="https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx47987fa108b8329b&redirect_uri="+
			wx_url+"&response_type=code&scope=snsapi_base&state=1#wechat_redirect";	
			return;
		}
	}
*/
		...
		success:function(data){
			...
			if(json.code=="success"){
				...
				var shareObject = {
					...
					desc: '情人节快到啦，快来亲亲我吧！',
					link: wx_url,
					imgUrl: 'http://www.fxzhoumo.com/redpacket/operation/valentine_img/share.jpg'
